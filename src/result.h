...
#define DEFINE_RESULT(T, E, NAME)                                              \
  ...
  int Result_is_ok(NAME *r) { return r->success; }                             \
  T Result_unwrap(NAME *r) {                                                   \
    if (r->success) {                                                          \
      return r->result;                                                        \
    } else {                                                                   \
      panicf("Attempted to unwrap Result<#T,#E>, but it contained an Err(%s)", \
             r->error);                                                        \
    }                                                                          \
  }                                                                            \
  E Result_unwrap_err(NAME *r) {                                               \
    if (!r->success) {                                                         \
      ...
      panicf("Attempted to unwrap error on Result<#T,#E>, but it contained "   \
             "an Ok()",                                                        \
             r->result);                                                       \
    }                                                                          \
  }
